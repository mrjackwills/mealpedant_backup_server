...
RUN ARCH=$(uname -m) && \
    ...
    && wget "https://github.com/mrjackwills/mealpedant_backup_server/releases/download/${CURRENT_VERSION}/mealpedant_backup_server_${SUFFIX}.tar.gz" \
    && tar xzvf "mealpedant_backup_server_${SUFFIX}.tar.gz" mealpedant_backup_server \
    && rm "mealpedant_backup_server_${SUFFIX}.tar.gz"
